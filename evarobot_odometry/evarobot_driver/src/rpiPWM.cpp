#include "../include/evarobot_driver/rpiPWM.h"
 
//Need to do this to be able to access these constants
//Private constants
const int rpiPWM::BCM2708_PERI_BASE;
const int rpiPWM::PWM_BASE;/* PWM controller */
const int rpiPWM::CLOCK_BASE; /* Clock controller */
const int rpiPWM::GPIO_BASE; /* GPIO controller */

const int rpiPWM::PWM_CTL ;

const int rpiPWM::PWM_RNG1;
const int rpiPWM::PWM_DAT1;

const int rpiPWM::PWM_RNG2;
const int rpiPWM::PWM_DAT2;

const int rpiPWM::PWMCLK_CNTL;
const int rpiPWM::PWMCLK_DIV;
const int rpiPWM::BLOCK_SIZE;


//Public constants
const int rpiPWM::PWMMODE;
const int rpiPWM::MSMODE;
const int rpiPWM::ERRFREQ;
const int rpiPWM::ERRCOUNT;
const int rpiPWM::ERRDUTY;
const int rpiPWM::ERRMODE;
 ...
 ***********************************************************************/
rpiPWM::rpiPWM(double Hz, unsigned int cnts, double duty,  int m)
{
...
void rpiPWM::setDirection(int pwm_mod, int _direction){
  if(pwm_mod == 0)
    this->direction0 = _direction;
  else
    this->direction1 = _direction;

  if(dirLast0 != direction0 || dirLast1 != direction1)
  {
    configPWMPin();
  }
 ...
 ****************************************************************************/
unsigned int rpiPWM::setDutyCycle(const double &duty, int pwm_no){
 ...
 * Return Value: 0 if successful or rpiPWM::ERRDUTY if Duty cycle is invalid 
 *******************************************************************************/
unsigned int rpiPWM::setDutyCycleForce(const double &duty, const int &m, int pwm_no){
	int retVal = 0;
	if( (m != PWMMODE) && (m != MSMODE) ) {
		retVal = ERRMODE;
	}
			...
			*(pwm + PWM_DAT1) = (int) ((this->dutyCycle/100.0) * this->counts);
			...
			if(this->mode == PWMMODE) //PWM mode
				...
				*(pwm + PWM_CTL) |= ( (1 << 7) | (1 << 0) );  
                        ...
                        // set the number of counts that constitute a period 
                        *(pwm + PWM_RNG2) = this->counts;
                        //set  duty cycle
                        *(pwm + PWM_DAT2) = (int) ((this->dutyCycle/100.0) * this->counts);
                        // start PWM1 in
                        if(this->mode == PWMMODE) //PWM mode
                                *(pwm + PWM_CTL) |= (1 << 8);
                        else // M/S Mode
 ...
 * Return Value:0 if successful or rpiPWM::ERRDUTY if Duty cycle is invalid 
 ***********************************************************************/
unsigned int rpiPWM::setDutyCycleCount(const unsigned int &dutyCycleCnts, int pwm_no){
   unsigned int retVal = 0;
...
unsigned int rpiPWM::setMode(const int &m){
	...
	if( (m != PWMMODE) && (m != MSMODE) ) {
		...
		this->mode = m;
		setDutyCycleForce(this->dutyCycle, this->mode, 0);
//		setDutyCycleForce(this->dutyCycle, this->mode, 1);
	 }
 ...
 *********************************************************************/
 double rpiPWM::getFrequency() const{ return this->frequency;}
 int rpiPWM::getCounts() const { return this->counts;}
 int rpiPWM::getDivisor() const {return this->divisor;}
 double  rpiPWM::getDutyCycle() const {return this->dutyCycle;}
 int rpiPWM::getMode() const{return this->mode;}
...
//  system("sudo chown 0:0 /dev/mem"); 
//system("sudo chmod u+s /dev/mem");
