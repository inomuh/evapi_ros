...
void IMDRIVER::CallbackWheelVel(const geometry_msgs::Twist::ConstPtr & msg)
{
	...
	pwm->SetDirection(0, f_left_wheel_velocity<0 ? 0:1);
	pwm->SetDirection(1, f_right_wheel_velocity<0 ? 0:1);
	
	int i_left_wheel_duty = int(fabs(f_left_wheel_velocity) * this->i_const_count);
	int i_right_wheel_duty = int(fabs(f_right_wheel_velocity) * this->i_const_count); 

	i_left_wheel_duty = i_left_wheel_duty>=(this->i_const_count-1) ? (this->i_const_count-1):i_left_wheel_duty;
	
	i_right_wheel_duty = i_right_wheel_duty>=(this->i_const_count-1) ? (this->i_const_count-1):i_right_wheel_duty;

	cout << "left_duty: " << i_left_wheel_duty << " right duty: " << i_right_wheel_duty << endl;

//	i_left_wheel_duty = this->i_const_count - 1;
//	i_right_wheel_duty = this->i_const_count - 1;
