				   ...
				   int i_mode)
{
	...
	this->i_const_count =  u_i_counts / this->f_max_lin_vel;
...
void IMDRIVER::CallbackVel(const geometry_msgs::Twist::ConstPtr & msg)
{
	...
	float f_left_wheel_velocity = f_linear_vel - f_angular_vel * f_wheel_separation / 2; // m/s
	float f_right_wheel_velocity = f_linear_vel + f_angular_vel * f_wheel_separation / 2; // m/s
...
int main(int argc, char **argv)
{
  ...
  n.param<string>("evarobot_driver/commandTopic", str_topic, "cmd_vel");
