...
/*
 * This function writes a byte of data "c_data" to a specific register
 * "c_register_address" in the I2C device This involves sending these two bytes
...
/*
 * This function writes two bytes of data "c_msb_data" and "c_lsb_data" to a specific register
 * "c_register_address" in the I2C device This involves sending these two bytes
 * in order to the i2C device by means of the ioctl() command. Since
 * both bytes are written (no read/write switch), both pieces
 * of information can be sent in a single message (i2c_msg structure)
...
/*
 * This function reads a byte of data "c_data" from a specific register
 * "c_register_address" in the I2C device. This involves sending the register
...
int IMI2C::ReadDataByte(char c_register_address, char & c_data)
{
	...
	messages[1].len = sizeof(*p_c_inbuff);
